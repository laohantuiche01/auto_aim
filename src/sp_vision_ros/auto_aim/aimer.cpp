...
namespace auto_aim
{
Aimer::Aimer(rclcpp::Node * node)
: left_yaw_offset_(std::nullopt), right_yaw_offset_(std::nullopt)
{
  yaw_offset_ = node->get_parameter("yaw_offset").as_double() / 57.3;        // degree to rad
  pitch_offset_ = node->get_parameter("pitch_offset").as_double() / 57.3;    // degree to rad
  comming_angle_ = node->get_parameter("comming_angle").as_double() / 57.3;  // degree to rad
  leaving_angle_ = node->get_parameter("leaving_angle").as_double() / 57.3;  // degree to rad
  ...
  // 可选的左右射击模式偏移
  if (node->has_parameter("left_yaw_offset") && node->has_parameter("right_yaw_offset")) {
    left_yaw_offset_ = node->get_parameter("left_yaw_offset").as_double() / 57.3;    // degree to rad
    right_yaw_offset_ = node->get_parameter("right_yaw_offset").as_double() / 57.3;  // degree to rad
