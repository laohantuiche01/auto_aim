...
Solver::Solver(rclcpp::Node * node) : R_gimbal2world_(Eigen::Matrix3d::Identity())
{
  auto R_gimbal2imubody_data = node->get_parameter("R_gimbal2imubody").as_double_array();
  auto R_camera2gimbal_data = node->get_parameter("R_camera2gimbal").as_double_array();
  auto t_camera2gimbal_data = node->get_parameter("t_camera2gimbal").as_double_array();
  R_gimbal2imubody_ = Eigen::Matrix<double, 3, 3, Eigen::RowMajor>(R_gimbal2imubody_data.data());
  R_camera2gimbal_ = Eigen::Matrix<double, 3, 3, Eigen::RowMajor>(R_camera2gimbal_data.data());
  t_camera2gimbal_ = Eigen::Matrix<double, 3, 1>(t_camera2gimbal_data.data());

  auto camera_matrix_data = node->get_parameter("camera_matrix").as_double_array();
  auto distort_coeffs_data = node->get_parameter("distort_coeffs").as_double_array();
