...
// ROS2 参数版本 - 暂不使用传统 Detector/Classifier
YOLOV8::YOLOV8(rclcpp::Node * node, bool debug)
: debug_(debug)
{
  model_path_ = node->get_parameter("yolov8_model_path").as_string();
  ...
  double scale, cv::Mat & output, const cv::Mat & bgr_img, int frame_count)
{
  ...
  for (auto it = armors.begin(); it != armors.end();) {
    ...
    if (classifier_) classifier_->classify(*it);
