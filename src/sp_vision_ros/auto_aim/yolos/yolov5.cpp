...
: debug_(debug), detector_(std::make_unique<Detector>(config_path, false))
{
...
// ROS2 参数版本 - 暂不使用传统 Detector
YOLOV5::YOLOV5(rclcpp::Node * node, bool debug)
: debug_(debug), use_traditional_(false)
{
  model_path_ = node->get_parameter("yolov5_model_path").as_string();
  device_ = node->get_parameter("device").as_string();
  binary_threshold_ = node->get_parameter("threshold").as_double();
  min_confidence_ = node->get_parameter("min_confidence").as_double();

  auto roi_x = node->get_parameter("roi_x").as_int();
  auto roi_y = node->get_parameter("roi_y").as_int();
  auto roi_width = node->get_parameter("roi_width").as_int();
  auto roi_height = node->get_parameter("roi_height").as_int();
  use_roi_ = node->get_parameter("use_roi").as_bool();

  roi_ = cv::Rect(roi_x, roi_y, roi_width, roi_height);
  offset_ = cv::Point2f(roi_x, roi_y);
  ...
  double scale, cv::Mat & output, const cv::Mat & bgr_img, int frame_count)
{
  ...
  for (auto it = armors.begin(); it != armors.end();) {
    ...
    // 使用传统方法二次矫正角点
    if (use_traditional_ && detector_) detector_->detect(*it, bgr_img);
